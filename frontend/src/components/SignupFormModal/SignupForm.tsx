...
const SignupForm = () => {
	...
	const clickSignup = async (e) => {
		...
		const res = await dispatch(signupUser({ email, password })).catch(async (res) => {
			let data;
			try {
	...
	const clickDemo = async (e) => {
		e.preventDefault();
		const res = await dispatch(loginUser({ email: 'demo@pin.drip', password: 'pindrip' }));
		if (res?.ok) navigate('/home');
	};
				...
				<div className="form-container">
					<form onSubmit={clickSignup}>
						...
						<div className="btn-holder">
							<input className="login-form-btn" type="submit" value="Continue" />
						</div>
						<div id="form-or-text">OR</div>
						<div className="btn-holder">
							<input className="demo-btn" type="submit" value="Log In as Demo User" onClick={clickDemo} />
						</div>
						<div id="form-terms-holder">
							<div id="form-terms-text">
								By continuing, you agree to Pindrip's{' '}
								<a
								...
								and acknowledge that your drip is absolutely immaculate.
							</div>
						</div>
						...
						<div id="not-member">
							<div>
								...
								<span onClick={replaceLogin} id="here-text">
									here
								</span>
								!
							</div>
